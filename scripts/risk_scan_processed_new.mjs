...
// ENV נדרש:
//   SUPABASE_URL
//   SUPABASE_SERVICE_ROLE_KEY     (או מפתח עם הרשאות SELECT לטבלת users_total)
//   OPENAI_API_KEY
//
// אופציונלי:
//   OPENAI_MODEL                  ברירת מחדל: gpt-5.2
//   BATCH_SIZE                    ברירת מחדל: 50
//   MAX_ITEMS                     ברירת מחדל: ללא הגבלה (0/ריק = ללא הגבלה)
...
if (!SUPABASE_KEY)
  throw new Error(
    "Missing env: SUPABASE_SERVICE_ROLE_KEY (or SUPABASE_KEY)"
  );
...
function buildSupabaseListNewUrl({ supabaseUrl, limit, offset }) {
  const base = supabaseUrl.replace(/\/$/, "");
  const params = new URLSearchParams();
  params.set("select", "id,phone,summarized_linked_talk_risk,processed");
  params.set("processed", "eq.NEW");
  params.set("order", "created_at.asc"); // אם אין created_at בטבלה שלך, הסדר לא קריטי
  params.set("limit", String(limit));
  params.set("offset", String(offset));
  return `${base}/rest/v1/users_total?${params.toString()}`;
}

async function supabaseListProcessedNew({ supabaseUrl, supabaseKey, limit, offset }) {
  const url = buildSupabaseListNewUrl({ supabaseUrl, limit, offset });
...
async function main() {
  ...
  while (true) {
    ...
    for (const row of rows) {
      ...
      const inputText = typeof row?.summarized_linked_talk_risk === "string"
    ...
    JSON.stringify(
      {
        ...
        response: results, // לפי הדרישה: "תוצאה בשדה: response"
      },
      null,
      2
    )
